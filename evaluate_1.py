...
df_test  = pd.read_csv("cleaned/crop_test.csv")
X_test   = df_test[FEATURES]
y_test   = df_test[TARGET]
le       = pickle.load(open("encoders/crop_label_encoder.pkl", "rb"))
...
for name in MODEL_NAMES:
    model  = pickle.load(open(f"models/crop_{name}.pkl", "rb"))
...
# Cross-validation
df_full = pd.read_csv("cleaned/crop_clean.csv")
cv = cross_val_score(best_model, df_full[FEATURES], df_full[TARGET], cv=5)
print(f"5-Fold CV: {cv.round(4)}  Mean: {cv.mean():.4f}")

pickle.dump(best_model, open("models/crop_best.pkl", "wb"))
print(f"\n✅ Best crop model saved → models/crop_best.pkl")